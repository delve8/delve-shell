...
func TestTryRelayMainEnter_nonSlashNoop(t *testing.T) {
	t.Helper()
	if TryRelayMainEnter("hello", 0, func(route.SlashSubmitPayload) bool { t.Fatal("relay called"); return true }) {
...
func TestTryRelayMainEnter_callsRelay(t *testing.T) {
	t.Helper()
	var saw bool
	ok := TryRelayMainEnter("/x", 3, func(p route.SlashSubmitPayload) bool {
		saw = true
